<?php
  ...
  $result = mysqli_query($con, "SELECT COURSE.courseID, COURSE.courseGrade, COURSE.courseTitle,
    ...
    COURSE.courseTime FROM COURSE, SCHEDULE, USER WHERE USER.userMajor IN (SELECT
      userMajor FROM USER WHERE userID = '$userID') AND USER.userID = SCHEDULE.userID AND
      SCHEDULE.courseID = COURSE.courseID GROUP BY
